...
//入力項目確認
$varify->name($name);
// del 2022/12/01 $varify->email($email);
$varify->time($start_hour, $start_min, $end_hour, $end_min);
$varify->todaysContent($todays_content);
// up 2022/12/01
	$varify->ampmContent($am_content,$pm_content);
	if(!$varify->ampmContent($am_content,$pm_content)){
		$am_content=$varify->amContent($am_content);
		$pm_content=$varify->pmContent($pm_content);
	}
// up 2022/12/01ここまで
$varify->myTyping($mytyping, $mytyping_skip);
$varify->sushida($sushida_course, $sushida_point, $sushida_skip);
$varify->todaysTerribly($todays_terribly);
$varify->todaysFelt($todays_felt);
$varify->opinion($opinion, $opinion_skip);
...
<body>
    ...
    <div id="form">
        ...
        <form action="
        	<?php
	        	if(strpos($place,'kashiwa')){
	        		echo 'https://docs.google.com/forms/u/1/d/e/1FAIpQLSf4Sk-jlV-sGMvAS-c1pPyskHZPHjcLEwVFBC-5Rvci7OdnXg/formResponse';
	        	}else{
	        		echo 'https://docs.google.com/forms/u/1/d/e/1FAIpQLSfAgyyT2cNyAja0wUN6RVdqnx59EslS9XRrwBeAJ0HWZSaVcQ/formResponse';
	        	}
        	?>
